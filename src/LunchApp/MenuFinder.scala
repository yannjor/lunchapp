...
object MenuFinder {
   ...
   * (for example if no internet). See "https://github.com/json4s/json4s" for how  json4s works.
   */
  
  
  def getMenus(url: String, restaurantName: String, language: String): String = {
    ...
    if (url.contains("fazer")) {
      try {
        ...
        val menuList = menusForToday.values.asInstanceOf[List[List[String]]].map(_.map(_.replaceAll("\n", "")))
        val menuListFixed = menuList.map(_.map(_.replaceAll("\n", ""))).map(_.map(_.replaceAll("\t", "")))    //Removes extra tabs and line breaks
        ...
        for (p <- menu) {
          p._1 match {
            case null => start = start + p._2.mkString("\n") + "\n\n"                     //If there is no menu title
            case _ => start = start + p._1 + ":" + "\n\n" + p._2.mkString("\n") + "\n\n"
          }
        } 
        start.split("\n").map("           " + _).mkString("\n") + "\n"
      } catch {
        case e: Throwable => "           " + restaurantName + date + "\n\n" + "           Error retrieving menus."
      }
      ...
      try {
        ...
        val menuListFixed = menuList.map(_.replaceAll("\t", "")).map(_.replaceAll("\n", ""))   //Removes extra tabs and line breaks
        ...
        for (i <- 0 until menuListFixed.length) {
          (json \ "courses")(i) \ "properties" match {
            ...
            case _ => properties += ((json \ "courses")(i) \ "properties").values.asInstanceOf[String]
          }
        }
      ...
      try {
        taffaMenu = {
          ...
          if (Calendar.getInstance().get(Calendar.DAY_OF_WEEK) != 1 && Calendar.getInstance().get(Calendar.DAY_OF_WEEK) != 7) {  //No menus on saturdays and sundays
            ("TÄFFÄ " + date + "\n" + taffa).split("\n").map("           " + _).mkString("\n\n")                           
          } else "           TÄFFÄ " + date + "\n\n" + "           No menus today."
        }
      } catch {
        case e: Throwable => taffaMenu = "           TÄFFÄ " + date + "\n\n" + "           Error retrieving menus."
      }    
      taffaMenu      
    } 
  }
